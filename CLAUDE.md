...
The persistence entity never leaves the service. `ContentletService` accepts a `ContentletDTO` and returns a `ContentletResponseDTO` (reads too — `findById`/`findAll`/`findByIds` all return response DTOs); `ContentletEntity` is created, saved, and mapped back entirely inside the service via `ContentletMapper` (`toEntity`/`toResponse`). The controller only deserializes the request into a `ContentletDTO`, performs the transport-level id checks (no id on POST, body-id-vs-URL-id on PUT), and forwards to the service — it never touches an entity or a repository. The request `ContentletDTO` and response `ContentletResponseDTO` share an `AbstractContentletDTO` base holding `id`, `contentType`, and the schemaless map via Jackson's `@JsonAnySetter`/`@JsonAnyGetter`.
