...
Reactive Spring Boot (WebFlux/Reactor) content management API on Java 21. Contentlets — schemaless content records — are persisted to MongoDB (system of record) and indexed into Elasticsearch for search. Maven build via the wrapper (`./mvnw`).
...
2. **MongoDB save** — `ContentletRepository` (reactive). Existence is checked first so the controller can return 201 vs 200.
...
Swagger UI is available via springdoc at `/webjars/swagger-ui/index.html`.
...
Container setup is shared through `testutils.MongoDBContainerUtils` / `ElasticSearchContainerUtils`: declare a `@Container static` field and register its URI in a `@DynamicPropertySource` method. Controller tests extend `AbstractContentletControllerTests` for a `WebTestClient` bound to the contentlets endpoint, and `@MockBean` the `ContentletIndexer` when ES isn't under test (stub helpers in `StubbingUtils`).
